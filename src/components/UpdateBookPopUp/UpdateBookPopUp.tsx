import styles from "./UpdateBookPopUp.module.css";
import { Htag } from "@/elements";
import { UpdateBookPopUpProps } from "@/components/UpdateBookPopUp/UpdateBookPopUp.props";
import { Form } from "@/components";

export const UpdateBookPopUp = ({ setOpen }: UpdateBookPopUpProps) => {
  return (
    <div className={styles.wrapper}>
      <div className={styles.popUpWrapper}>
        <div onClick={() => setOpen(false)} className={styles.closePopUp}>
          <svg
            width="48"
            height="48"
            viewBox="0 0 24 24"
        ...
        <div>
          <Form action="update" titleColor="black" />
        </div>
      </div>
    </div>
  );
